...
DayNightTimelinePanel::Result DayNightTimelinePanel::Draw(EditorScene& scene, EditorAssets& assets,
    ...
    if(ImGui::BeginChild("##TimelineKeys",{details,0},true)){
        ...
        if(m_selectedKey>=0&&m_selectedKey<(int)m_timeline.keys.size()){
            ...
            const auto audio=assets.ContentAssetPaths(EditorAssets::Type::Audio);if(ImGui::BeginCombo("Ambient Audio",FileName(k.ambientAudioPath))){if(ImGui::Selectable("None",k.ambientAudioPath.empty())){k.ambientAudioPath.clear();k.ambientAudioId={};changed=true;}for(const auto&p:audio){ImGui::PushID(p.c_str());if(ImGui::Selectable(FileName(p),p==k.ambientAudioPath)){k.ambientAudioPath=p;k.ambientAudioId=assets.AssetIdForPath(p);changed=true;}ImGui::PopID();}ImGui::EndCombo();}
