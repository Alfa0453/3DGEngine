...
void ProceduralScatterGraphPanel::DrawDetails(){
    ...
    case engine::ScatterNodeType::MeshOutput:{const std::string preview=n->meshPath.empty()?"Choose static mesh...":std::filesystem::path(n->meshPath).filename().string();if(ImGui::BeginCombo("Mesh",preview.c_str())){for(const auto&mesh:m_meshes){ImGui::PushID(mesh.path.c_str());if(ImGui::Selectable(mesh.relative.c_str(),mesh.path==n->meshPath)){n->meshPath=mesh.path;n->meshId=mesh.id;m_dirty=true;}ImGui::PopID();}ImGui::EndCombo();}m_dirty|=ImGui::DragFloat("Weight",&n->weight,.05f,0,100,"%.2f");break;}
    }
