...
AssetDependencyViewerPanel::Result AssetDependencyViewerPanel::Draw(const engine::AssetRegistry& registry,const std::string& contentRoot,bool* open){
    ...
    if(ImGui::BeginCombo("##DependencyType",preview)){if(ImGui::Selectable("All asset types",m_typeFilter==0))m_typeFilter=0;for(int i=1;i<=static_cast<int>(engine::AssetType::Combat);++i)if(ImGui::Selectable(engine::AssetTypeName(static_cast<engine::AssetType>(i)),m_typeFilter==i))m_typeFilter=i;ImGui::EndCombo();}
...
void AssetDependencyViewerPanel::DrawListView(){const auto* n=m_graph.Find(m_selected);if(!n)return;if(ImGui::BeginTable("DependencyLists",2,ImGuiTableFlags_Borders|ImGuiTableFlags_RowBg)){ImGui::TableSetupColumn("Referenced by");ImGui::TableSetupColumn("Depends on");ImGui::TableHeadersRow();const size_t rows=std::max(n->incoming.size(),n->outgoing.size());for(size_t i=0;i<rows;++i){ImGui::PushID(static_cast<int>(i));ImGui::TableNextRow();ImGui::TableSetColumnIndex(0);if(i<n->incoming.size()){const auto* x=m_graph.Find(n->incoming[i]);ImGui::PushID("Incoming");if(ImGui::Selectable(x?Leaf(x->asset.virtualPath).c_str():"Missing"))m_selected=n->incoming[i];ImGui::PopID();}ImGui::TableSetColumnIndex(1);if(i<n->outgoing.size()){const auto* x=m_graph.Find(n->outgoing[i]);ImGui::PushID("Outgoing");if(ImGui::Selectable(x?Leaf(x->asset.virtualPath).c_str():"Missing"))if(x)m_selected=n->outgoing[i];ImGui::PopID();}ImGui::PopID();}ImGui::EndTable();}}
