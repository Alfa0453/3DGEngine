...
AssetDependencyViewerPanel::Result AssetDependencyViewerPanel::Draw(const engine::AssetRegistry& registry,const std::string& contentRoot,bool* open){
    ...
    if(ImGui::BeginCombo("##DependencyType",preview)){if(ImGui::Selectable("All asset types",m_typeFilter==0))m_typeFilter=0;for(int i=1;i<=static_cast<int>(engine::AssetType::Spawn);++i)if(ImGui::Selectable(engine::AssetTypeName(static_cast<engine::AssetType>(i)),m_typeFilter==i))m_typeFilter=i;ImGui::EndCombo();}
