...
public:
    struct Result {
        ...
        bool refreshFontsRequested   = false;   // rescan the content folder for .3dgfont assets
        std::string path;                       // current path text (for load/save/set)
    };

    // Draws the panel window. 'open' is the ImGui close flag. 'imageChoices' are
    // content-folder image paths for the Image dropdown; 'fontChoices' are content-folder
    // .3dgfont paths for the Text/Button Font dropdown; 'texLookup' resolves a path to a
    // GL texture id so the canvas can preview images.
...
private:
    ...
    char m_fontFilter[128] = "";    // substring filter for the font dropdown
    bool m_dirty = false;
};
